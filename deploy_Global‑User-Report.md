...
This guide accompanies **run\_global\_user\_report.py**, a flexible wrapper around `daily_odin_report.py` that can export

* **Global User Report of All Service Providers**
...
### 5. **Make the wrapper executable**
   ...
   ```bash
   chmod +x run_global_user_report.py
   ```
...
### 8. **Deployment for one or more SPs**
   ...
   # Run User Report for All Service Providers
...
### 11. **Add the Cron entry - Examples**
    ...
    20 2 * * * /usr/bin/env bash -c 'cd /opt/odin && source venv/bin/activate && ./run_global_user_report.py >> /var/log/odin/users_all.log 2>&1'

    # 2) Weekly export for ACME & CONTOSO every Monday (03:05)
    5 3 * * 1 /usr/bin/env bash -c 'cd /opt/odin && source venv/bin/activate && ./run_global_user_report.py -s acme contoso >> /var/log/odin/users_acme_contoso.log 2>&1'
    ```
